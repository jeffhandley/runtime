...
**⚠️ MANDATORY**: Every agentic workflow in this repository **must** utilize the Copilot PAT Pool as detailed in `.github/workflows/shared/pat_pool.README.md`. This mechanism selects a random Copilot PAT from a numbered pool of secrets to avoid rate-limiting from a single shared PAT, and runs agentic jobs in an isolated `copilot-pat-pool` environment.
...
Both the workflow-level `environment: copilot-pat-pool` and the `with.environment: copilot-pat-pool` import input are required so the agentic job and the `pat_pool` selection job run in the same isolated environment.

If other `engine` properties are customized for the workflow (for example `model`), add them into this same `engine` block. Hand-editing can rearrange the PAT pool frontmatter and comment for ideal maintainability.
